...
export default new SlashCommand({
    ...
    run: async ({ interaction }) => {
        ...
        if (!voiceChannel) return interaction.reply({ content: 'Nie jesteś na żadnym kanale głosowym, ani kanał do dołączenia nie został podany!' })
            .catch(err => logger.warn({ message: 'Could not replay', silent: true, }));
        ...
        if(voiceChannel.type === ChannelType.GuildVoice) {
            ...
            client.distube.voices.join(voiceChannel).catch((err) => {
                ...
                return interaction.reply({ content: 'Coś poszło nie tak i nie udało mi się dołączyć na kanał głosowy!' })
                    .catch(err => logger.warn({ message: 'Could not replay', silent: true, }));
            })
            .then(async () => {
